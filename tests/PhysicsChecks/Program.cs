...
try { Checks.Run(); NativeFireChecks.Run(); PacketChecks.Run(game); StartupChecks.Run(); NeighborHeatChecks.Run(); ConfigChecks.Run(); SettingsChecks.Run(); SmolderChecks.Run(); GuideReviewChecks.Run(); InventorySecurityChecks.Run(); PeatChecks.Run(); PeatIntegrationChecks.Run(); BrownDependencyChecks.Run(); } catch (Exception e) { Console.WriteLine(e); Environment.Exit(1); }
